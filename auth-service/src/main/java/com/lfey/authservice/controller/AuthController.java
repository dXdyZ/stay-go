...
public class AuthController implements AuthControllerDocs {
    ...
    //Использовать если клиент будет браузерный
//    @PostMapping("/logout")
...
//    }
    ...
    @PostMapping("/verify")
    public ResponseEntity<?> getAuthInfoAndValidation(@RequestHeader("Authorization") String authHeader) {
        String token = null;
        ...
        return ResponseEntity.ok()
                .header("X-User-PublicId", authInfo.publicId().toString())
    ...
    public ResponseEntity<UserDetailsDto> validationUpdateEmail(@Valid @RequestBody ValidationCodeDto validationCodeDto,
        ...
        return ResponseEntity.ok(userService.updateEmailInUserService(validationCodeDto, publicId));
    }
}
